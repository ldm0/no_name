...
fn main() {
    ...
    if args.len() < 2 {
        panic!("You need to specify a media file.");
    }
    ...
    let result =
        unsafe { avcodec_parameters_to_context(pCodecContext, pCodecParameters) };
    if result < 0 {
    ...
    let pFrame = unsafe { av_frame_alloc().as_mut() }.expect("failed to allocated memory for AVFrame");
    let pPacket = unsafe { av_packet_alloc().as_mut() }.expect("failed to allocated memory for AVPacket");
...
fn decode_packet(
    ...
    while response >= 0 {
        ...
        if response >= 0 {
            ...
            let frame_filename = format!("frame-{}.pgm", pCodecContext.frame_number);
